...
PMDA「過去1週間以内に更新された添付文書情報」を毎日 7時・17時 に自動で取り込み、
...
## 仕組み(ざっくり)
```
毎日 7時・17時(GitHub Actions) ─→ 1week.html を取得
...
| `.github/workflows/daily.yml` | 毎日の自動実行設定(7時・17時) | 時刻変更など |
...
- 自動実行は2本: 毎日 7:00と17:00 `daily-pmda-watch`(添付文書ウォッチ。PMDAの掲載が平日15時台のため17時の回で当日分を拾う)と 毎日0:15 `if-index`(インタビューフォーム一覧)。同時には走らない設定(concurrency)
