...
- データの作り方: `data/tenpu_index.json`(添付文書一覧)のうち錠剤(YJ剤形コードF)・カプセル(M)の添付文書XMLを1件ずつ取得し、
...
- 更新: PC側タスクの毎回の実行(`run.py --shikibetsu`)で「新しく増えた文書+添付文書の更新日が変わった文書」だけを最大300件/回で自動取得。
  初回の全件(4000件超・数時間)や取り直しは手動で `python src/shikibetsu_index.py`(途中で止めても次回続きから。`--retry-errors` で過去のエラー分もやり直し)
- 二重実行防止: 実行中は `data/.shikibetsu.lock` を置き、他の実行はスキップされる(6時間より古いロックは無視)
- 制限: 添付文書に識別コード欄が無い製品(無地の錠剤・記載が画像のみ等)は出てこない。カプセルと同じ剤形コードの外用薬(軟膏チューブ等)の印字コードも一部収録される
