...
トップ(ホーム)  docs/index.html        ← 入口。項目は site/home.json で管理(1項目=1カード)
 └ 💊 薬剤師ツールボックス  docs/toolbox/   ← 添付文書ウォッチのまとめ + ツール一覧
     ...
     ├ 📘 インタビューフォーム検索 docs/if/ ← 薬剤名→候補→クリックでIF(PDF)。データは data/if_index.json(手動更新)
...
| `src/if_index.py` | インタビューフォーム一覧をPMDAから集める(手動実行) | ロジック変更時のみ |
| `src/run.py` | 実行入口 | - |
...
| `.github/workflows/if-index.yml` | インタビューフォーム一覧の取り直し(手動ボタンのみ) | - |
...
- 一覧データ(`data/if_index.json`)は **毎日は更新しない**(必要なときに使う機能なので)。作り直したいとき:
