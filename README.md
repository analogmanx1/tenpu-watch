...
| `.github/workflows/daily.yml` | クラウド側の自動実行(公開の保険) | 時刻変更など |
...
- GitHub Actionsは2本: 毎日 7:00と17:00 `daily-pmda-watch`(公開の保険。クラウドからは中身が取れないため通信失敗3連続で打ち切る設計。PMDAがブロックをやめたらそのまま復活する)と 毎日0:15 `if-index`(インタビューフォーム一覧。こちらはPMDAの検索サイト側なので現状クラウドから取れている)。同時には走らない設定(concurrency)
