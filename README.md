...
**術前休薬・禁忌チェック**(添付文書に「手術時は禁忌/休薬・中止」などの記載がある薬の一覧)も同じサイトにある。
...
トップ(ホーム)「ファーマビット」 docs/index.html ← 入口。サイト名・項目は site/home.json で管理(1項目=1カード)
 └ 💊 薬剤師ツールボックス  docs/toolbox/   ← 添付文書ウォッチのまとめ + ツール一覧
     ...
     ├ 🏥 術前休薬・禁忌チェック docs/shujutsu/ ← 添付文書に手術時の禁忌・休薬などの記載がある薬の一覧(一般名ごと・絞り込みつき)。データは data/shujutsu_index.json(PC側タスクが毎日差分更新)
...
| `src/shujutsu_index.py` | 添付文書XMLの決まった章から「手術に触れている文」を集める+仕分ける(PC側タスクが識別コードのついでに毎日差分更新) | 拾い方・仕分けを変えたいとき |
...
| `data/shujutsu_index.json` | 手術に触れている文の候補(文書ごと)。`python src/shujutsu_index.py` で差分更新 | ✕(コマンドで更新) |
...
## 術前休薬・禁忌チェック(docs/shujutsu/。2026-09-20)
- 使い方: 一覧ページを開くと、添付文書に手術時の記載がある薬が一般名ごとに並ぶ(薬効分類順/五十音順)。薬剤名(一般名・販売名)・薬効・文中の言葉(例: `14日`)で絞り込み。
  区分ボタンで出し分け: **🚫 禁忌**(禁忌の章に手術の記載) / **⏸ 手術時の対応あり**(休薬・中止・◯日前・用量調節などの記載) / **📎 その他**(それ以外の手術関連の注意。最初は畳んである)。
  黄色マーカー=手術に触れている文。「出典 ↗」でその添付文書PDF、「販売名 ◯件」で同じ一般名の各製品の添付文書PDFが開く
- **載るのは「添付文書に手術の記載がある薬」だけ**。現場で術前休薬する薬でも、添付文書に記載が無ければ出ない(例: 2026-09時点でシロスタゾールは記載なし)。院内の休薬一覧やガイドラインの代わりではなく「添付文書上の根拠を引く一覧」
- データの作り方は2段構え(`src/shujutsu_index.py`):
  1. **取得時(広めに保存)**: 全添付文書のXMLから、警告・禁忌・効能/用法に関連する注意・重要な基本的注意・特定の背景・相互作用・適用上の注意・その他の注意の章で
     「手術/術前/術中/術後/周術期」を含む文を候補として `data/shujutsu_index.json` に保存(章はXMLのタグで見分ける。効能・副作用・臨床成績などは見ない=「手術不能乳癌」等のノイズ源)。
     9章の見出し(「手術を予定している患者」等)がヒットしたら直後の説明文も添える。相互作用の表は併用薬の名前を添える
  2. **サイト生成時(仕分け)**: 候補から適応症名・既往歴などの言い回し(`NOT_SURG`)を除き、禁忌の章→🚫、対応の言葉(`ACT`: 中止・休薬・◯日前・増量…)あり→⏸、残り→📎 に分ける。
     手術で「使う側」の薬(`USED_IN_SURGERY`: 全身麻酔剤111・催眠鎮静剤112・局所麻酔剤121・骨格筋弛緩剤122)は📎から外す。一般名でまとめ、メーカーごとの細かい言い回し違いは似た文を束ねて一番多い表現を代表にする
  - → **仕分けの条件を直したら `python src/run.py --build-only` だけで反映**(PMDAから取り直さない)。広めの網(`NET`)や見る章(`SKIP_TAGS`)を変えたときだけ `DATA_VERSION` を上げる(=全文書取り直し・3〜5時間)
- 更新: PC側タスクの毎回の実行(`run.py --shikibetsu`)で、**識別コードの取得で落としたXMLをそのまま受け取る**(1文書を2回取りに行かない)。受け取れなかった改版ぶんは最大300件/回で自分で取得。
  初回の全件(約1.1万件・約3時間)や取り直しは手動で `python src/shujutsu_index.py`(途中で止めても次回続きから。二重実行防止は `data/.shujutsu.lock`)
- `data/shujutsu_index.json` の書き手は**PC側だけ**(衝突時は ours。`local_update.ps1` の Resolve-StuckMerge に登録済み)
