...
- 使い方: 錠剤・カプセルに印字されている記号(例: `DK 505`)を入力 → 候補をクリックすると添付文書(PDF)が開く。
  スペース・ハイフン・全角半角・大文字小文字は自動で吸収(`dk505` でもOK)。薬剤名・会社名を足してAND絞り込みも可
- データの作り方: `data/tenpu_index.json`(添付文書一覧)の**全文書**のXMLを1件ずつ取得し、
...
- 制限: 添付文書に識別コード欄が無い製品(無地の錠剤・記載が画像のみ等)は出てこない。軟膏チューブ等、錠剤以外の印字コードも添付文書に載っていれば収録される
